...
import { Target, CheckSquare, BookOpen, TrendingUp, Plus, Trash2 } from 'lucide-react';
          ...
          <div className="space-y-6">
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-orange-500/20">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setShowEventModal(true)} className="flex items-center gap-2 px-3 py-1 rounded-lg hover:opacity-90 transition-all text-white text-sm font-bold" style={{ backgroundColor: '#FF6200' }}>
                  <Plus size={16} />Add
                </button>
              </div>
              ...
              <div className="grid grid-cols-7 gap-2">
                {Array.from({ length: 35 }, (_, i) => {
                  ...
                  return (
                    <div key={i} className={`aspect-square p-2 rounded-lg ${!isCurrentMonth ? 'opacity-30' : ''} ${isToday ? 'ring-2 ring-orange-500' : ''} bg-black/20 hover:bg-black/30 transition-all cursor-pointer`}>
                      {isCurrentMonth && (
                        <>
                          <div className={`text-sm font-bold mb-1 ${isToday ? 'text-orange-500' : ''}`}>{dayNum}</div>
                          <div className="space-y-1">
                            {dayEvents.slice(0, 2).map(event => (
                              <div key={event.id} className="text-xs px-1 py-0.5 rounded truncate" style={{ backgroundColor: '#FF6200' }}>{event.title}</div>
                            ))}
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-orange-500/20">
              ...
              <div className="grid grid-cols-7 gap-3">
                {Array.from({ length: 35 }, (_, i) => {
                  ...
                  return (
                    <div key={i} className={`min-h-32 p-3 rounded-lg ${!isCurrentMonth ? 'opacity-30' : ''} ${isToday ? 'ring-2 ring-orange-500' : ''} bg-black/20 hover:bg-black/30 transition-all cursor-pointer`}>
                      {isCurrentMonth && (
                        <>
                          <div className={`text-lg font-bold mb-2 ${isToday ? 'text-orange-500' : ''}`}>{dayNum}</div>
                          <div className="space-y-1">
                            {dayEvents.map(event => (
                              <div key={event.id} className="text-xs px-2 py-1 rounded truncate" style={{ backgroundColor: '#FF6200' }}>
